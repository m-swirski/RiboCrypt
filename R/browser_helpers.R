multiOmicsPlot_bottom_panels <- function(reference_sequence, display_range, annotation,
  ...
  seq_panel_hits <- createSeqPanelPattern(target_seq[[1]],
  ...
  return(list(
    seq_panel = seq_panel, gene_model_panel = gene_model_panel,
...
multiOmicsPlot_all_track_plots <- function(profiles, withFrames, colors, ylabels,
  ...
  if (frames_type == "animate") {
    plots <- list(getPlotAnimate(rbindlist(profiles, idcol = "file"),
      withFrames = withFrames[1],
  ...
  if (summary_track) {
    nplots <- nplots + 1
    plots <- make_summary_track(
      profiles, plots, withFrames, colors,
...
multiOmicsPlot_complete_plot <- function(track_panel, bottom_panel, display_range,
  ...
  if (without_sequence_track) { # plotly subplot without sequence track
    ...
    nplots <- nplots + 3
    plots <- c(plots, list(
      automateTicks(nt_area_template()),
  ...
  plots <- lapply(plots, function(x) {
    x %>% layout(
      xaxis = list(title = list(font = list(size = 22)), tickfont = list(size = 16)),
      yaxis = list(
        title = list(font = list(size = 22)), tickfont = list(size = 16),
  ...
  if (!without_sequence_track) {
    multiomics_plot <- addJSrender(
      multiomics_plot, bottom_panel$target_seq,
...
multiOmicsPlot_internal <- function(display_range, df, annotation = "cds", reference_sequence = findFa(df),
  ...
  bottom_panel <- multiOmicsPlot_bottom_panels(
    reference_sequence, display_range, annotation,
    start_codons, stop_codons, custom_motif,
    custom_regions, viewMode
  )
  ...
  profiles <- multiOmicsPlot_all_profiles(
    display_range, reads, kmers,
    kmers_type, frames_type, frames_subset,
    withFrames, log_scale, BPPARAM, normalization
  )

  track_panel <- multiOmicsPlot_all_track_plots(
    profiles, withFrames, colors, ylabels,
    ylabels_full_name, bottom_panel$lines,
    frames_type, total_libs,
    summary_track, summary_track_type,
    BPPARAM
  )
  return(multiOmicsPlot_complete_plot(
    track_panel, bottom_panel, display_range,
    proportions, seq_render_dist,
    display_sequence, display_dist,
    aa_letter_code, input_id, plot_name,
    plot_title, width, height, export.format
    ...
    if (valid_zoom) {
      ...
      if (tx_coord_interval) {
        ...
        if (as.numeric(width(ir)) > 0) {
          zoom_range <- c(
            max(as.numeric(start(ir))[1] - 10, 1),
