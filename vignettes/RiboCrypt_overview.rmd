...
Browser window is created with `multiOmicsPlot` functions. First, lets have a look at a single Ribo-seq profile made with a single library (cds is extended by 30 bases). In the resulting plot you can zoom in and inspect coverage:

```{r eval = TRUE, echo = TRUE, message = FALSE}

  multiOmicsPlot_ORFikExp(extendLeaders(extendTrailers(cds[3], 30), 30), annotation = cds,df = df[11,],
...
If entire experiment is used, all libraries are displayed, what can make the browsing window too crowded to be clear.
```{r eval = TRUE, echo = TRUE, message = FALSE}

  multiOmicsPlot_ORFikExp(extendLeaders(extendTrailers(cds[3], 30), 30), annotation = cds,df = df,
                        frames_type = "columns")
...
```{r eval = TRUE, echo = TRUE, message = FALSE}
  ...
  multiOmicsPlot_ORFikExp(extendLeaders(extendTrailers(cds[3], 30), 30), annotation = cds,df = df[which(df$libtype == "RFP"),],
                        frames_type = "lines")
